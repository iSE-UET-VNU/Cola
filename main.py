...
def run_process(data_type, dataset_name, noise_type, error_rate, knn_k, n_iterations, sampling_rate, encode_model):
    if not os.path.exists(f"backend/Data/{norm_name(dataset_name)}/{norm_name(encode_model)}.npy"):
        if data_type == 'image':
            encode_model, dataset_name = encode.extract_img_feature(dataset_name=dataset_name, batch_size=64, encode_model=encode_model)
        elif data_type == 'text':
            encode_model, dataset_name = encode.extract_text_feature(dataset_name=dataset_name, batch_size = 64, encode_model=encode_model)
        else:
            raise ValueError(f"Data type should be either 'image' or 'text'. Got {data_type}.")
    else:
        encode_model, dataset_name = norm_name(encode_model), norm_name(dataset_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--noise_type', type=str, default='asym', help='Type of noise (sym, asym, ins)')
    parser.add_argument('--error_rate', type=float, default=0.45, help='Error rate should be less than 1')
    parser.add_argument('--k', type=int, default=21, help='Parameter for KNN detection')
    parser.add_argument('--iteration', type=int, default=4, help='Number of global iterations')
